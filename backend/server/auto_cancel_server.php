...
// Auto-cancel pending-for-approval that already reached/passed start time
function autoCancelPendingApprovals($conn) {
    try {
        $currentDateTime = date('Y-m-d H:i:s');

        $conn->beginTransaction();

        // Pick reservations whose CURRENT active status is pending approval (1 or 8)
        // Exclude if any active status is in (4, 6, 9) per requirement, or already cancelled (5)
        // Start time must be reached/passed
        $sql = "
            ...
            INNER JOIN tbl_reservation_status rs_active
                ON rs_active.reservation_reservation_id = r.reservation_id
               AND rs_active.reservation_active = 1
               AND rs_active.reservation_status_status_id IN (1, 8) -- pending admin approval or pending department approval
            WHERE r.reservation_start_date IS NOT NULL
              AND r.reservation_start_date <= :current_datetime
              AND NOT EXISTS (
                    SELECT 1 FROM tbl_reservation_status s4
                    WHERE s4.reservation_reservation_id = r.reservation_id
                      AND s4.reservation_active = 1
                      AND s4.reservation_status_status_id IN (4, 5, 6, 9)
              )
        ";
        ...
        foreach ($toCancel as $reservation) {
            ...
            if ($insertStmt->execute()) {
                // Deactivate any active pending statuses (1 or 8)
                ...
                // Notification
                $notificationMessage = "Your reservation '{$reservation['reservation_title']}' has been automatically cancelled because it was still pending for approval when the scheduled start time was reached.";
                ...
                sendPushNotificationToUser(
                    ...
                    "Your reservation '{$reservation['reservation_title']}' was auto-cancelled because it remained pending for approval past its start time.",
                    [
                ...
                echo "[" . date('Y-m-d H:i:s') . "] Auto-cancelled PENDING reservation ID: {$reservation['reservation_id']} - Title: {$reservation['reservation_title']} - User: {$reservation['full_name']}\n";
            }
        }
        ...
        if ($cancelledCount > 0) {
            echo "[" . date('Y-m-d H:i:s') . "] SUCCESS: Auto-cancelled {$cancelledCount} pending approval reservations past start time\n";
        }

        return [
            'status' => 'success',
            'message' => "Auto-cancelled {$cancelledCount} pending approval reservations",
        ...
        error_log('Database error in autoCancelPendingApprovals: ' . $e->getMessage());
...
echo "Checking for expired reschedules and pending approvals every 60 seconds...\n";
...
while (true) {
    try {
        ...
        if ($currentTime - $lastCheck >= $checkInterval) {
            ...
            $resultPending = autoCancelPendingApprovals($conn);
            
            if ($result['status'] === 'success' && $result['cancelled_count'] == 0) {
                echo "[" . date('Y-m-d H:i:s') . "] No expired reschedules found\n";
            }
            if ($resultPending['status'] === 'success' && $resultPending['cancelled_count'] == 0) {
                echo "[" . date('Y-m-d H:i:s') . "] No overdue pending approvals found\n";
            }
