...
class FacultyStaff {
    ...
    public function handleCancelReservation($reservationId, $userId) {
        try {
            ...
            // Check if reservation has status_id 9 (Ongoing) - cannot cancel ongoing reservations
            $sqlCheckOngoing = "
                SELECT COUNT(*) as status_count 
                FROM tbl_reservation_status 
                WHERE reservation_reservation_id = :reservation_id 
            ...
            $stmtCheckOngoing->bindParam(':reservation_id', $reservationId, PDO::PARAM_INT);
            $stmtCheckOngoing->execute();
            $ongoingCheck = $stmtCheckOngoing->fetch(PDO::FETCH_ASSOC);
            
            // If reservation has status_id 9 (Ongoing), prevent cancellation
            if ($ongoingCheck['status_count'] > 0) {
                $this->conn->rollBack();
                return json_encode([
                    'status' => 'error',
            ...
            // Check if status ID 6 is still active and within time range
            ...
            $sqlDeactivate = "
                ...
                WHERE reservation_reservation_id = :reservation_id 
                AND reservation_status_status_id IN (6)";
            ...
            // $sqlDepartmentNotification = "INSERT INTO tbl_notification_reservation 
    ...
    public function createReservation($data, $type) {
        ...
        try {
            ...
            if ($userLevelId == 3 || $userLevelId == 6 || $userLevelId == 16 || $userLevelId == 17) {
                ...
                // Department approvals will be created when admin clicks "Process" button
                // This is now handled in handleProcessed method in reservation.php
                error_log("Reservation created - Department approvals will be added when processed by admin");
                ...
                // Department approvals will be created when admin clicks "Process" button
                // This is now handled in handleProcessed method in reservation.php
                error_log("Reservation created (Levels 5/18) - Department approvals will be added when processed by admin");
    ...
    public function updateReservationDetails($reservationId, $title, $description, $userId) {
        try {
            $this->conn->beginTransaction();
            ...
            // Update the reservation title and description
            $sqlUpdate = "UPDATE tbl_reservation 
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    switch ($operation) {
        ...
        case "updateReservationDetails":
            ...
            if ($reservationId === null || $title === null || $description === null) {
                echo json_encode(['status' => 'error', 'message' => 'reservation_id, title, and description are required']);
                break;
            }
            echo $facultyStaff->updateReservationDetails($reservationId, $title, $description, $userId);
            break;
