...
class Reservation {
    ...
    public function fetchAvailableVenues($startDateTime = null, $endDateTime = null, $excludeIds = null) {
        ...
        try {
            ...
            if ($startDateTime && $endDateTime && !empty($filteredVenues)) {
                ...
                // Debug: Log the input parameters for troubleshooting
                error_log("DEBUG fetchAvailableVenues - Input params:");
                ...
                error_log("Reschedule venue IDs during specified time: " . (empty($rescheduleVenues) ? 'none' : implode(',', $rescheduleVenues)));
                
                // Combine both reserved and reschedule venues
                $allBlockedVenues = array_unique(array_merge($reservedVenues, $rescheduleVenues));
                error_log("All blocked venue IDs: " . (empty($allBlockedVenues) ? 'none' : implode(',', $allBlockedVenues)));
    ...
    public function updateReservationReschedule($reservation_id, $reschedule_start_date, $reschedule_end_date, $user_admin_id = null, $venue_ids = null, $vehicle_ids = null) {
        try {
            ...
            error_log("[updateReservationReschedule] Starting with reservation_id: {$reservation_id}, venue_ids: " . json_encode($venue_ids) . ", vehicle_ids: " . json_encode($vehicle_ids));
            ...
            error_log("Current status for reservation {$reservation_id}: " . json_encode($currentStatus));
            ...
            if (!empty($venue_ids) && is_array($venue_ids)) {
                error_log("[updateReservationReschedule] Using provided venue_ids for conflict check");
                foreach ($venue_ids as $venueId) {
                    $venues[] = ['reservation_venue_venue_id' => $venueId];
                }
            } else {
                error_log("[updateReservationReschedule] Fetching venue_ids from database");
            ...
            error_log("[updateReservationReschedule] Checking conflicts for " . count($venues) . " venues");
            
            // Check conflicts for each venue
            foreach ($venues as $venue) {
                $venueId = $venue['reservation_venue_venue_id'];
                error_log("[updateReservationReschedule] Checking venue ID: {$venueId}");
                $conf = $this->hasConflictRequest('venue', $venueId, $reschedule_start_date, $reschedule_end_date, 0, $reservation_id);
                error_log("[updateReservationReschedule] Venue {$venueId} conflict count: " . $conf['count']);
                if ($conf['count'] > 0 && !empty($conf['conflicts'])) {
                    $venueName = $conf['conflicts'][0]['venue_name'] ?? 'Venue ID ' . $venueId;
                    error_log("[updateReservationReschedule] ⚠️ Venue conflict found: {$venueName}");
                    $conflictResults[] = [
                        'type' => 'Venue',
                        'name' => $venueName,
                        'resource_id' => $venueId,
            ...
            if (!empty($vehicle_ids) && is_array($vehicle_ids)) {
                error_log("[updateReservationReschedule] Using provided vehicle_ids for conflict check");
                foreach ($vehicle_ids as $vehicleId) {
                    $vehicles[] = ['reservation_vehicle_vehicle_id' => $vehicleId];
                }
            } else {
                error_log("[updateReservationReschedule] Fetching vehicle_ids from database");
            ...
            error_log("[updateReservationReschedule] Checking conflicts for " . count($vehicles) . " vehicles");
            
            // Check conflicts for each vehicle
            foreach ($vehicles as $vehicle) {
                $vehicleId = $vehicle['reservation_vehicle_vehicle_id'];
                error_log("[updateReservationReschedule] Checking vehicle ID: {$vehicleId}");
                $conf = $this->hasConflictRequest('vehicle', $vehicleId, $reschedule_start_date, $reschedule_end_date, 0, $reservation_id);
                error_log("[updateReservationReschedule] Vehicle {$vehicleId} conflict count: " . $conf['count']);
                if ($conf['count'] > 0 && !empty($conf['conflicts'])) {
                    $vehicleName = $conf['conflicts'][0]['vehicle_name'] ?? 'Vehicle ID ' . $vehicleId;
                    error_log("[updateReservationReschedule] ⚠️ Vehicle conflict found: {$vehicleName}");
                    $conflictResults[] = [
                        'type' => 'Vehicle',
                        'name' => $vehicleName,
            ...
            if (!empty($conflictResults)) {
                error_log("[updateReservationReschedule] ❌ CONFLICTS DETECTED for reservation {$reservation_id}:");
                error_log("[updateReservationReschedule] Conflict details: " . json_encode($conflictResults, JSON_PRETTY_PRINT));
            ...
            error_log("[updateReservationReschedule] ✓ NO CONFLICTS found for reservation {$reservation_id}");
            error_log("[updateReservationReschedule] Checked resources - Venues: " . count($venues) . ", Vehicles: " . count($vehicles) . ", Equipment: " . count($equipment));
            error_log("[updateReservationReschedule] Date range: {$reschedule_start_date} to {$reschedule_end_date}");
            ...
            if ($status7Exists['count'] == 0) {
                error_log("Status 7 not found for reservation {$reservation_id}, inserting it now");
                ...
                error_log("Status 7 already exists for reservation {$reservation_id}, skipping insertion");
            }

            // 3) Insert status_id 3 with active=1
            error_log("Inserting status 3 for reservation {$reservation_id}");
            ...
            try {
                ...
                if ($reservationData && $reservationData['reservation_user_id']) {
                    ...
                    error_log("Push notification sent to user {$reservationData['reservation_user_id']} for rescheduled reservation {$reservation_id}");
                }
            } catch (Exception $e) {
    ...
    private function checkHolidayConflict($startDate, $endDate) {
        try {
            ...
            if (!empty($holidays)) {
                error_log("Holiday conflict detected: " . json_encode($holidays));
                return [
                    'has_holiday' => true,
        ...
        try {
            ...
            switch ($resourceType) {
                ...
                case 'equipment':
                    ...
                    error_log('Equipment check equip_id=' . (int)$resourceId . ' remaining=' . $remainingQuantity . ' requested=' . (int)$requestedQuantity . ' total_capacity=' . (int)$capacityResult['total_capacity']);
    ...
    public function handleProcessed($reservationId, $userId) {
        try {
            ...
            error_log("Current status for reservation {$reservationId}: " . json_encode($currentStatus));
            ...
            if (in_array($currentStatusId, $pendingStatuses)) {
                ...
                try {
                    ...
                    error_log("🔍 Processing department approvals for reservation ID: {$reservationId}");
                    
                    // Get reservation details including user info
                    $reservationSql = "
                        SELECT 
                            r.reservation_user_id,
                    ...
                    if ($reservationData) {
                        $requesterUserId = (int)$reservationData['reservation_user_id'];
                        $userLevelId = (int)$reservationData['users_user_level_id'];
                        $userDeptId = (int)$reservationData['users_department_id'];
                        
                        error_log("Reservation requester - User ID: {$requesterUserId}, Level: {$userLevelId}, Department: {$userDeptId}");
                        
                        // Check if user is GSD Secretary
                        $isGsdSecretary = false;
                        if ($userLevelId == 6) {
                            $deptCheckSql = "SELECT departments_name FROM tbl_departments WHERE departments_id = :dept_id";
                            $deptCheckStmt = $this->conn->prepare($deptCheckSql);
                            $deptCheckStmt->bindParam(':dept_id', $userDeptId, PDO::PARAM_INT);
                            $deptCheckStmt->execute();
                            $deptData = $deptCheckStmt->fetch(PDO::FETCH_ASSOC);
                            if ($deptData && strtoupper($deptData['departments_name']) === 'GSD') {
                                $isGsdSecretary = true;
                            }
                        }
                        ...
                        $venuesSql = "
                            SELECT reservation_venue_venue_id 
                            FROM tbl_reservation_venue 
                            WHERE reservation_reservation_id = :reservation_id
                        ";
                        $venuesStmt = $this->conn->prepare($venuesSql);
                        $venuesStmt->bindParam(':reservation_id', $reservationId, PDO::PARAM_INT);
                        $venuesStmt->execute();
                        $venues = $venuesStmt->fetchAll(PDO::FETCH_COLUMN);
                        
                        if (!empty($venues)) {
                            error_log("Found " . count($venues) . " venue(s) for reservation ID: {$reservationId}");
                            
                            // Get required departments for all venues from tbl_venue_department_approval
                            ...
                            if ($venueApprovalStmt) {
                                ...
                                error_log("Required departments from venue approval: " . json_encode(array_column($requiredDepartments, 'departments_name')));
                                ...
                                if (!empty($requiredDepartments)) {
                                    ...
                                    error_log("Venues have department approval requirements - using venue-based logic");
                                    ...
                                    error_log("Departments from venue approval: " . json_encode($departmentsToApprove));
                                    
                                    // Exclude user's own department for certain user levels
                                    if (in_array($userLevelId, [5, 6, 18, 20]) && $userDeptId > 0) {
                                        $departmentsToApprove = array_filter($departmentsToApprove, function($deptId) use ($userDeptId) {
                                            return (int)$deptId !== $userDeptId;
                                        });
                                        error_log("Excluded user's department ID {$userDeptId} from approvals for user level {$userLevelId}");
                                    }
                                    ...
                                    if ($exclusiveStmt->execute()) {
                                        ...
                                        if (!empty($exclusiveResults)) {
                                            ...
                                            if (in_array($userLevelId, [5, 6, 18, 20]) && $userDeptId > 0) {
                                                $exclusiveResults = array_filter($exclusiveResults, function($deptId) use ($userDeptId) {
                                                    return (int)$deptId !== $userDeptId;
                                                });
                                                error_log("Filtered out user's own department ID {$userDeptId} from approval_exclusive results for user level {$userLevelId}");
                                            }
                                            
                                            if (!empty($exclusiveResults)) {
                                                $departmentsToApprove = array_merge($departmentsToApprove, $exclusiveResults);
                                                error_log("Added additional departments from approval_exclusive: " . json_encode($exclusiveResults));
                                            }
                                        }
                                    }
                                    ...
                                    error_log("Venues have NO department approval requirements");
                                ...
                                if (!empty($departmentsToApprove)) {
                                    error_log("Inserting department approvals for departments: " . implode(', ', $departmentsToApprove));
                            ...
                            error_log("No venues found for reservation ID: {$reservationId} - no department approvals needed");
                        }
                    } else {
                        error_log("⚠️  Could not fetch reservation details for ID: {$reservationId}");
                    }
                    ...
                    error_log("Successfully inserted status 7 and updated status 1 to active for reservation {$reservationId} by user {$userId}");
                    error_log("🔍 DEBUG: About to check hasDepartmentApproval for reservation ID {$reservationId}");
                    ...
                    error_log("🔍 DEBUG: hasDepartmentApproval result for reservation ID {$reservationId}: " . ($hasDepartmentApproval ? 'true' : 'false'));
                    
                    if ($hasDepartmentApproval) {
                        error_log("🔔 Reservation ID: $reservationId has department approval requirements - sending push notifications");
                        $this->sendPushNotificationToDepartmentApproval($reservationId);
                        error_log("✅ Department approval push notification process completed for reservation ID: $reservationId");
                    } else {
                        error_log("ℹ️  Reservation ID: $reservationId has no department approval requirements - no push notifications needed");
                    }
    ...
    private function hasDepartmentApproval($reservationId) {
        try {
            error_log("🔍 DEBUG: hasDepartmentApproval function called for reservation ID: {$reservationId}");
            ...
            error_log("🔍 DEBUG: Department approval query result for reservation ID {$reservationId}: " . json_encode($result));
            
            $hasApproval = $result['count'] > 0;
            error_log("🔍 DEBUG: hasDepartmentApproval returning: " . ($hasApproval ? 'true' : 'false') . " for reservation ID {$reservationId}");
    ...
    private function sendPushNotificationToDepartmentApproval($reservationId) {
        try {
            ...
            if (empty($allDeptUsers)) {
                error_log("⚠️  No department users with push subscriptions found for reservation ID: $reservationId - no notifications will be sent");
                return;
            }
            
            error_log("👥 Found " . count($allDeptUsers) . " department user(s) with push subscriptions for reservation ID: $reservationId");
            ...
            foreach ($allDeptUsers as $user) {
                ...
                if ($error || $httpCode < 200 || $httpCode >= 300) {
                    error_log("❌ Push notification FAILED for department user {$user['users_id']} ({$user['full_name']}) in department {$user['departments_name']}: " . ($error ?: "HTTP $httpCode"));
                    $errorCount++;
                } else {
                    error_log("✅ Push notification SUCCESS for department user {$user['users_id']} ({$user['full_name']}) in department {$user['departments_name']}: HTTP $httpCode");
                    $successCount++;
                }
            }

            error_log("📊 DEPARTMENT APPROVAL PUSH NOTIFICATION SUMMARY:");
            error_log("   ✅ Successful notifications: $successCount");
            error_log("   ❌ Failed notifications: $errorCount");
            error_log("   👥 Total department users notified: " . count($allDeptUsers));
            error_log("   🎯 Reservation ID: $reservationId");
            
            if ($successCount > 0) {
                error_log("🎉 SUCCESS: Department approval push notifications sent successfully!");
            }
            
            if ($errorCount > 0) {
                error_log("⚠️  WARNING: Some department approval push notifications failed to send");
            }
    ...
    if (isset($data['operation'])) {
        switch ($data['operation']) {
            ...
            case "updateReservationReschedule":
                ...
                error_log("[Switch Case] updateReservationReschedule - venue_ids: " . json_encode($venue_ids) . ", vehicle_ids: " . json_encode($vehicle_ids));
