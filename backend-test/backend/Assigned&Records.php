    ...
    public function fetchAllAssignedReleases() {
        try {
            ...
            foreach (array_merge($venueData, $vehicleData, $equipmentData) as $row) {
                ...
                if (empty($rid)) {
                    error_log("[fetchAllAssignedReleases] Skipping row with empty reservation_reservation_id");
                    continue;
                }
                ...
                error_log("[fetchAllAssignedReleases] Processing row type: $rowType for reservation $rid");
            ...
            foreach ($reservations as $rid => &$res) {
                // Debug: Log array counts before cleanup
                error_log("[fetchAllAssignedReleases] BEFORE CLEANUP - Reservation $rid: " . 
                         count($res['venues']) . " venues, " . 
                         count($res['vehicles']) . " vehicles, " . 
                         count($res['equipments']) . " equipments");
                ...
                $res['vehicles'] = array_values(array_filter($res['vehicles'], function($item) use ($rid) {
                    ...
                    if (!$isVehicle) {
                        $itemType = isset($item['name']) ? 'venue' : 'unknown';
                        error_log("[fetchAllAssignedReleases] Cleanup: Removed $itemType from vehicles array in reservation $rid");
                    }
                    return $isVehicle;
                }));
                ...
                $res['venues'] = array_values(array_filter($res['venues'], function($item) use ($rid) {
                    $isVenue = isset($item['reservation_venue_id']) 
                              ...
                              && !isset($item['vehicle_model_id']);
                    if (!$isVenue) {
                        $itemType = isset($item['vehicle_license']) ? 'vehicle' : 'unknown';
                        error_log("[fetchAllAssignedReleases] Cleanup: Removed $itemType from venues array in reservation $rid");
                    }
                    return $isVenue;
                }));
                ...
                $res['equipments'] = array_values(array_filter($res['equipments'], function($item) use ($rid) {
                    ...
                    if (!$isEquipment) {
                        $itemType = 'unknown';
                        if (isset($item['vehicle_license'])) $itemType = 'vehicle';
                        elseif (isset($item['reservation_venue_id'])) $itemType = 'venue';
                        error_log("[fetchAllAssignedReleases] Cleanup: Removed $itemType from equipments array in reservation $rid");
                    }
                    return $isEquipment;
                }));
                
                // Debug: Log array counts after cleanup
                error_log("[fetchAllAssignedReleases] AFTER CLEANUP - Reservation $rid: " . 
                         count($res['venues']) . " venues, " . 
                         count($res['vehicles']) . " vehicles, " . 
                         count($res['equipments']) . " equipments");
            }
            ...
            if (!empty($allReservationIds)) {
                ...
                error_log('[fetchAllAssignedReleases] equipment conditions=' . count($equipCondData));
            }
            ...
            if (!empty($allEqIds)) {
                ...
                error_log('[fetchAllAssignedReleases] unit conditions=' . count($unitCondData));
            }
            ...
            if (!empty($allVehicleIds)) {
                ...
                error_log('[fetchAllAssignedReleases] vehicle conditions=' . count($vehicleCondData));
            }
            ...
            if (!empty($allVenueIds)) {
                ...
                error_log('[fetchAllAssignedReleases] venue conditions=' . count($venueCondData));
            }
    ...
    public function insertUnits($equipIds, $quantities, $reservationId, $startDate, $endDate, $userId) {
        try {
            ...
            if ($userInfo) {
                ...
                error_log("insertUnits: User bypass check - UserID: $userId, Level: $userLevelName ($userLevelId), Department: $departmentName ($userDepartmentId), CanBypass: " . ($canBypassEquipmentRestrictions ? 'YES' : 'NO'));
            }
            
            // Log the start of unit allocation
            error_log("insertUnits: Starting allocation for reservation_id: $reservationId, equipIds: " . json_encode($equipIds) . ", quantities: " . json_encode($quantities) . ", canBypass: " . ($canBypassEquipmentRestrictions ? 'true' : 'false'));
            ...
            for ($i = 0; $i < count($equipIds); $i++) {
                ...
                if ($equipType === 'bulk') {
                    ...
                    // BUT if user can bypass restrictions, skip this check entirely
                    ...
                    if (!$canBypassEquipmentRestrictions) {
                        ...
                        error_log("insertUnits: BYPASS MODE - Skipping conflict check for other reservations due to user privileges");
                    }
                    ...
                    // For bypass users, auto-assign all available units up to the requested quantity
                    if ($canBypassEquipmentRestrictions) {
                        // When bypassing, assign all available units up to quantity needed
                        $actualUnitsToInsert = array_slice($availableUnits, 0, $quantity);
                        error_log("insertUnits: BYPASS MODE - Auto-assigning " . count($actualUnitsToInsert) . " units for equipment $equipId (requested: $quantity)");
                    } else {
                        $actualUnitsToInsert = array_slice($availableUnits, 0, $quantity);
                    }
                    ...
                    if (count($actualUnitsToInsert) === 0) {
                        ...
                        error_log("insertUnits: No available units for equipment $equipId" . ($canBypassEquipmentRestrictions ? " (bypass mode)" : ""));
                        $results[] = [
                            'equip_id' => $equipId,
                            'reservation_equipment_id' => $reservationEquipmentId,
                            'type' => 'serialized',
