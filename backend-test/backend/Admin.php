...
public function fetchEquipmentsWithStatus() {
    ...
    public function fetchDriver($startDateTime = null, $endDateTime = null, $userId = null, $reservationId = null) {
        try {
            // Log function call
            error_log("fetchDriver called with startDateTime: " . ($startDateTime ?? 'null') . ", endDateTime: " . ($endDateTime ?? 'null') . ", reservationId: " . ($reservationId ?? 'null'));
            // Base query to get all active drivers - only name fields
            $sql = "
                SELECT 
                    u.users_id,
                    u.users_fname,
            ...
            if ($reservationId !== null) {
                ...
                error_log("fetchDriver: Excluding current reservation ID: $reservationId from conflict check");
            }
            ...
            if (!$bypassReservationCheck) {
                try {
                    ...
                    error_log("fetchDriver: Unavailable drivers query executed successfully");
                    error_log("fetchDriver: Raw unavailable driver IDs: " . print_r($unavailableDriverIds, true));
                } catch (PDOException $e) {
                    error_log("fetchDriver: Error checking unavailable drivers - " . $e->getMessage());
                    // If query fails, assume no drivers are unavailable (safer to show more drivers)
                    $unavailableDriverIds = [];
                }
            } else {
                $unavailableDriverIds = [];
                error_log("fetchDriver: Bypass enabled, skipping unavailable driver check");
            }
            ...
            error_log("fetchDriver: Final SQL: " . $sql);
    ...
    public function updateProfile($userData){
        try {
            ...
            // Debug logging for title_id and users_suffix
            error_log("UpdateProfile Debug - title_id: " . var_export($params['title_id'] ?? 'NOT_SET', true));
            error_log("UpdateProfile Debug - users_suffix: " . var_export($params['users_suffix'] ?? 'NOT_SET', true));
            error_log("UpdateProfile Debug - SQL: " . "UPDATE tbl_users SET " . implode(', ', $updateFields) . " WHERE users_id = :userId");
...
public function updateEquipmentUnit($unitData) {
    try {
        ...
        if ($activeTransactionCheck['hasActive']) {
            return json_encode([
                'status' => 'error', 
                'message' => 'Cannot update equipment unit with active reservations: ' . implode(', ', $activeTransactionCheck['resourcesWithTransactions'])
            ]);
        }
...
public function updateVehicleLicense($vehicleData) {
    try {
        ...
        if ($activeTransactionCheck['hasActive']) {
            return json_encode([
                'status' => 'error', 
                'message' => 'Cannot update vehicle with active reservations: ' . implode(', ', $activeTransactionCheck['resourcesWithTransactions'])
            ]);
        }
...
public function updateVenue($venueData) {
    try {
        ...
        if ($activeTransactionCheck['hasActive']) {
            return json_encode([
                'status' => 'error', 
                'message' => 'Cannot update venue with active reservations: ' . implode(', ', $activeTransactionCheck['resourcesWithTransactions'])
            ]);
        }
...
public function venueExists($venueName) {
    ...
    public function updateDriver($reservation_driver_id, $reservation_driver_user_id, $driver_name = null) {
        ...
        error_log("updateDriver method - reservation_driver_id: " . var_export($reservation_driver_id, true));
        error_log("updateDriver method - reservation_driver_user_id: " . var_export($reservation_driver_user_id, true));
        error_log("updateDriver method - driver_name: " . var_export($driver_name, true));
        ...
        try {
            ...
            if ($reservation_driver_user_id === 'custom') {
                ...
                error_log("updateDriver: Converted 'custom' driver to null user_id, driver_name: " . var_export($driver_name, true));
            }
            ...
            // Debug the binding values
            error_log("Binding reservation_driver_id: " . var_export($reservation_driver_id, true));
            error_log("Binding reservation_driver_user_id: " . var_export($reservation_driver_user_id, true));
            error_log("Binding driver_name: " . var_export($driver_name, true));
    ...
    public function insertDriver($reservation_driver_user_id, $reservation_vehicle_id = null, $driver_name = null, $reservation_driver_id = null) {
        // Debug logging inside the method
        error_log("insertDriver method - reservation_driver_user_id: " . var_export($reservation_driver_user_id, true));
        error_log("insertDriver method - reservation_vehicle_id: " . var_export($reservation_vehicle_id, true));
        error_log("insertDriver method - driver_name: " . var_export($driver_name, true));
        error_log("insertDriver method - reservation_driver_id: " . var_export($reservation_driver_id, true));
        
        try {
            // Handle custom driver case - when driverId is 'custom', treat as null for user_id
            if ($reservation_driver_user_id === 'custom') {
                $reservation_driver_user_id = null;
                error_log("insertDriver: Converted 'custom' driver to null user_id, driver_name: " . var_export($driver_name, true));
            }
            ...
            error_log("Binding reservation_driver_user_id: " . var_export($reservation_driver_user_id, true));
            error_log("Binding driver_name: " . var_export($driver_name, true));
            error_log("Binding reservation_vehicle_id: " . var_export($reservation_vehicle_id, true));
            ...
            if ($stmt->execute()) {
                ...
                if ($reservation_driver_user_id && $reservationId) {
                    try {
                        ...
                        error_log("Notification inserted for driver $reservation_driver_user_id for reservation $reservationId");
                        ...
                        error_log("Push notification sent to driver $reservation_driver_user_id for reservation $reservationId");
...
// Check if equipment category exists
    ...
    public function fetchVehicleCategories() {
        $sql = "SELECT vehicle_category_id, vehicle_category_name FROM tbl_vehicle_category WHERE is_active = 1 ORDER BY vehicle_category_name";
        return $this->executeQuery($sql);
    }
...
private function isFinalApprover($reservationId, $userId) {
    try {
        ...
        error_log("Approval sequence check - Total approvers: " . count($approvers));
        error_log("Approval sequence data: " . json_encode($approvers));
        
        if (empty($approvers)) {
            error_log("No approval sequence found, treating as final approver");
        ...
        error_log("Final approver check - UserId: $userId, LastApproverUserId: $lastApproverUserId, IsFinal: " . ($isFinal ? 'true' : 'false'));
...
private function getNextApprover($reservationId, $currentUserId) {
    try {
        ...
        error_log("Getting next approver - Current UserId: $currentUserId");
        error_log("Available approvers: " . json_encode($approvers));
        ...
        if ($currentSequence === null) {
            error_log("Current user not found in approval sequence");
            return null;
        }
...
public function handleRequest($reservationId, $isAccepted, $userId, $notificationMessage = '', $notification_user_id = null) {
    try {
        ...
        // Debug logging
        error_log("HandleRequest Debug: ReservationId=$reservationId, UserId=$userId, IsAccepted=" . ($isAccepted ? 'true' : 'false'));
        ...
        if ($isAccepted) {
            ...
            error_log("Is final approver check: " . ($isFinalApprover ? 'true' : 'false'));
        }
        ...
        if ($isAccepted) {
            ...
            error_log("Updated pending status to active. Rows affected: " . $stmtUpdate->rowCount());
        ...
        if (!$isAccepted) {
            ...
            if (!$isFinalApprover) {
                ...
                error_log("Skipped notification to user 99 - final approver declined (User ID: $userId, Reservation ID: $reservationId)");
            }
            ...
            if (!$isFinalApprover) {
                ...
                error_log("Skipped notification to user 99 - already final approver (User ID: $userId, Reservation ID: $reservationId)");
            }
...
public function reactivateEquipmentUnits($unitIds, $userId = null) {
    ...
    public function updateApprovalOrder($approvalOrderId, $usersId, $approvalSequence) {
        try {
            ...
            if ($result && $result['active_count'] > 0) {
                return json_encode([
                    'status' => 'error', 
                    'message' => 'Cannot update approval order while there are ' . $result['active_count'] . ' active reservation(s) in the system. Please wait until all reservations are completed, cancelled, or declined.'
                ]);
            }
    ...
    public function deleteApprovalOrder($approvalOrderId) {
        try {
            ...
            if ($result && $result['active_count'] > 0) {
                return json_encode([
                    'status' => 'error', 
                    'message' => 'Cannot delete approval order while there are ' . $result['active_count'] . ' active reservation(s) in the system. Please wait until all reservations are completed, cancelled, or declined.'
                ]);
            }
    ...
    public function updateAllApprovalOrders($approvalOrders) {
        try {
            ...
            if (!empty($existingOrderIds)) {
                $usageCheck = $this->checkApprovalOrderUsage($existingOrderIds);
                if ($usageCheck['hasActive']) {
                    return json_encode([
                        'status' => 'error', 
                        'message' => 'Cannot update approval sequence while there are active reservations in the system. Please wait until all reservation request marked as Reserved/Declined.'
                    ]);
                }
            }
        ...
        try {
            ...
            if (!empty($existingOrderIds)) {
                ...
                if ($usageCheck['hasActive']) {
                    return json_encode([
                        'status' => 'error', 
                        'message' => 'Cannot delete all approval orders with active reservations: ' . implode(', ', $usageCheck['resourcesWithTransactions'])
                    ]);
                }
            }
    ...
    try {
        ...
        // Debug logging
        error_log("Approval Order Check - Query Results: " . print_r($results, true));
        error_log("Approval Order Check - Input IDs: " . print_r($approvalOrderIds, true));
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    switch ($operation) {
        ...
        case "insertDriver":
            ...
            // Debug logging
            error_log("insertDriver - reservation_driver_user_id: " . var_export($reservation_driver_user_id, true));
            error_log("insertDriver - reservation_vehicle_id: " . var_export($reservation_vehicle_id, true));
            error_log("insertDriver - driver_name: " . var_export($driver_name, true));
            error_log("insertDriver - reservation_driver_id: " . var_export($reservation_driver_id, true));
        ...
        case "fetchDriver":
            ...
            echo $user->fetchDriver($startDateTime, $endDateTime, $userId, $reservationId);
            break;
