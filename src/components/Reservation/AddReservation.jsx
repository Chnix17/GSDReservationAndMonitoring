...
const AddReservation = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        const encryptedUserLevel =
          SecureStorage.getSessionItem("user_level_id");
        const decryptedUserLevel = parseInt(encryptedUserLevel);
        console.log("this is encryptedUserLevel", encryptedUserLevel);
        if (
          decryptedUserLevel !== 3 &&
          decryptedUserLevel !== 15 &&
          decryptedUserLevel !== 16 &&
          decryptedUserLevel !== 17 &&
          decryptedUserLevel !== 18 &&
          decryptedUserLevel !== 5 &&
          decryptedUserLevel !== 6
        ) {
          localStorage.clear();
          navigate("/gsd");
        }
  ...
  const validateCurrentStep = () => {
    switch (currentStep) {
      case 0:
        if (!resourceType) {
          toast.error(
            "Please select a resource type (Venue, Vehicle, or Equipment)",
          );
          return false;
        }
        return true;
      ...
      case 1:
        ...
        if (
          ...
          Object.keys(equipmentQuantities).length === 0
        ) {
      ...
      case 3:
        if (resourceType === "venue") {
          ...
          if (!formData.eventTitle || !formData.description) {
            toast.error(
              "Please fill in all required equipment reservation fields",
            );
            return false;
          }
          return true;
      ...
      <>
        {formData.resourceType === "vehicle" && selectedModels.length === 0 && (
          <div style={{ color: "red", marginBottom: 16 }}>
            <b>Warning:</b> No vehicles selected. Please select at least one
            vehicle to see driver options.
          </div>
        )}
    ...
    try {
      ...
      if (resourceType === "venue") {
        ...
        const venuePayload = {
          ...
          form_data: {
            ...
            participants: formData.participants
              ? formData.participants.toString()
              : "0",
            user_id: userId,
            venues: formData.venues,
            equipment: Object.entries(selectedVenueEquipment)
        ...
        if (response.data.status === "success") {
          ...
          throw new Error(
            response.data.message || "Failed to submit venue reservation",
          );
        }
      } else if (resourceType === "vehicle") {
        ...
        if (formData.driverType === "own") {
          ...
          if (
            !formData.ownDrivers ||
            formData.ownDrivers.length !== selectedModels.length ||
            formData.ownDrivers.some((d) => !d.name.trim())
          ) {
          ...
          if (
            !formData.mixedDrivers ||
            formData.mixedDrivers.length !== selectedModels.length
          ) {
          ...
          const ownDriversWithoutNames = formData.mixedDrivers.filter(
            (d) => d.driverType === "own" && !d.name.trim(),
          );
          if (ownDriversWithoutNames.length > 0) {
            toast.error(
              'Please enter driver names for vehicles marked as "Own Driver"',
            );
            return false;
          }
          ...
          if (formData.forceMixedDrivers) {
            ...
            const defaultDrivers = formData.mixedDrivers.filter(
              (d) => d.driverType === "default",
            ).length;

            if (defaultDrivers > availableDrivers) {
              toast.error(
                `Cannot assign more than ${availableDrivers} default drivers. Please adjust your assignments.`,
              );
              return false;
            }
            ...
            if (formData.mixedDrivers.length === 0) {
              toast.error(
                "Please configure driver assignments for all vehicles",
              );
              return false;
            }
          }
        ...
        const vehiclePayload = {
          ...
          form_data: {
            ...
            ...(formData.driverType === "own" || formData.driverType === "mixed"
              ? { drivers }
              : {}),
            equipment: Object.entries(selectedVenueEquipment)
              .map(([equipId, quantity]) => ({
        ...
        console.log(
          "Vehicle Payload:",
          JSON.stringify(vehiclePayload, null, 2),
        );
        ...
        if (response.data.status === "success") {
          ...
          throw new Error(
            response.data.message || "Failed to submit vehicle reservation",
          );
        }
      } else if (resourceType === "equipment") {
        ...
        if (response.data.status === "success") {
          ...
          throw new Error(
            response.data.message || "Failed to submit equipment reservation",
          );
        }
      }
    } catch (error) {
      ...
      toast.error(
        error.message || "An error occurred while submitting the reservation",
      );
      return false;
    } finally {
      setLoading(false);
  ...
  const renderReviewSection = () => {
    const selectedVenues = venues.filter((v) =>
      formData.venues.includes(v.ven_id),
    );
    ...
    >
      <Result
        ...
        extra={[
          ...
          <AntButton key="new" onClick={resetForm} icon={<PlusOutlined />}>
        ...
        <div className="space-y-4">
          <ReservationCalendar
            onDateSelect={function (dateData, endDateParam) {
              ...
              toast.success(
                "Date and time selected successfully! Please fill in the required details.",
              );
            }}
            selectedResource={{
              type: resourceType,
  ...
  const handlePrintRequest = () => {
    ...
    const selectedDriver = availableDrivers.find(
      (d) => d.driver_id.toString() === formData.driverName?.toString(),
    );
              ...
              <div>
                ...
                <div className="flex items-center gap-2">
                  <input
                    ...
                    onKeyDown={(e) => {
                      if (e.key === "Enter") handleAddLocalPassenger();
                    }}
  ...
  const EquipmentSelectionModal = ({
    ...
    useEffect(() => {
      if (modalOpenRef.current && !modalJustOpenedRef.current) {
        console.log(
          "Modal opened, resetting local state. Previous state:",
          localEquipmentQuantities,
        );
        setLocalState({});
        setEquipmentSearch("");
    ...
    useEffect(() => {
      if (
        ...
        localEquipmentQuantities &&
        Object.keys(localEquipmentQuantities).length > 0
      ) {
        console.log(
          "Updating local state from props:",
          localEquipmentQuantities,
        );
    ...
    const handleLocalQuantityChange = (equipId, value) => {
      ...
      const maxAvailable =
        Number(equip.available_quantity || equip.available) || 0;

      // Ensure quantity doesn't exceed available amount and is not negative
      const constrainedValue = Math.max(
        0,
        Math.min(numericValue, maxAvailable),
      );
      ...
      setLocalState((prev) => {
        ...
        if (constrainedValue === 0) {
          ...
          console.log(
            "Adding/updating equipment in state:",
            equipmentKey,
            "quantity:",
            constrainedValue,
          );
    ...
    const handleConfirm = () => {
      ...
      // Show success message
      const selectedCount = Object.values(localState).filter(
        (qty) => qty > 0,
      ).length;
      if (selectedCount > 0) {
        toast.success(
          `${selectedCount} equipment item${selectedCount > 1 ? "s" : ""} selected successfully`,
        );
      }
    };
    ...
    const EquipmentCard = ({
      ...
      const availableQuantity =
        parseInt(item.available_quantity || item.available) || 0;
                ...
                <div className="flex items-center justify-center gap-2 mt-auto pt-2">
                  <button
                    onClick={(e) => {
                      ...
                      console.log(
                        "Decreasing quantity for equipment:",
                        item.equip_id,
                        "from",
                        currentQuantity,
                        "to",
                  ...
                  <button
                    onClick={(e) => {
                      ...
                      console.log(
                        "Increasing quantity for equipment:",
                        item.equip_id,
                        "from",
                        currentQuantity,
                        "to",
              ...
              {currentEquipment.map((item) => {
                ...
                return (
                  <EquipmentCard
                    ...
                    onClick={() => {
                      const availableQty =
                        parseInt(item.available_quantity || item.available) ||
                        0;
  ...
  const renderDriverDropdown = (selectedModels, vehicles, setFormData) => {
    ...
    return (
      <Form.Item
        label={
          <span className="text-sm">
            Driver Information <span className="text-red-500">*</span>
          </span>
        }
        required
      >
        <div className="space-y-3">
          {shouldForceOwnDrivers && (
            <div className="p-3 bg-amber-50 border border-amber-200 rounded-lg">
              ...
              <p className="text-xs text-amber-700">
                You must provide your own drivers for this reservation due to no
                available drivers.
              </p>
            </div>
          )}
          ...
          {shouldForceMixedDrivers && (
            <div className="p-3 bg-blue-50 border border-blue-200 rounded-lg">
              <div className="flex items-center gap-2 mb-2">
                ...
                >
                  <path
                    strokeLinecap="round"
                    strokeLinejoin="round"
                    d="M13.5 16.875h3.375m0 0h3.375m-3.375 0V13.5m0 3.375v3.375M6 10.5h2.25a2.25 2.25 0 002.25-2.25V6a2.25 2.25 0 00-2.25-2.25H6A2.25 2.25 0 004 6v2.25A2.25 2.25 0 006 10.5zm9 0h2.25a2.25 2.25 0 002.25-2.25V6a2.25 2.25 0 00-2.25-2.25H15A2.25 2.25 0 0013.5 6v2.25A2.25 2.25 0 0015 10.5z"
                  />
                </svg>
              ...
              <p className="text-xs text-blue-700">
                Some drivers are available. You can use default drivers for some
                vehicles and provide your own drivers for the rest.
              </p>
            </div>
          )}
          ...
          <Radio.Group
            ...
            onChange={(e) => {
              setFormData((prev) => ({
                ...
                ownDrivers:
                  e.target.value === "own"
                    ? safeSelectedModels.map((vehicle_id) => {
                        // Try to preserve existing names if switching back and forth
                        const existing = prev.ownDrivers?.find(
                          (d) => d.vehicle_id === vehicle_id,
                        );
                        return {
                          vehicle_id,
                ...
                mixedDrivers:
                  e.target.value === "mixed"
                    ? safeSelectedModels.map((vehicle_id) => {
                        ...
                        const existing = prev.mixedDrivers?.find(
                          (d) => d.vehicle_id === vehicle_id,
                        );
                        return {
                          vehicle_id,
          ...
          {formData.driverType === "own" && (
            <div className="space-y-2">
              ...
              {safeSelectedModels.length > 0
                ? safeSelectedModels.map((vehicle_id, idx) => {
                    // Find vehicle info
                    const vehicle = safeVehicles.find(
                      (v) => v.vehicle_id === vehicle_id,
                    );
                    const driverObj = formData.ownDrivers?.find(
                      (d) => d.vehicle_id === vehicle_id,
                    ) || { name: "" };
                    return (
                      ...
                      <Input
                        ...
                        onChange={(e) => {
                          ...
                          setFormData((prev) => ({
                            ...prev,
                            ownDrivers: [
                              { vehicle_id: "forced", name: newName },
                            ],
                          }));
                        }}
                        className="rounded"
          ...
          {formData.driverType === "mixed" && (
            <div className="space-y-2">
              ...
              {formData.driverType === "mixed" &&
                formData.mixedDrivers.length > 0 && (
                  <div
                    className={[
                      "p-2 bg-gray-50 border border-gray-200 rounded-lg",
                      (() => {
                        const defaultCount = formData.mixedDrivers.filter(
                          (d) => d.driverType === "default",
                        ).length;
                        const availableDrivers = formData.availableDrivers || 0;
                        return defaultCount > availableDrivers
                  ...
                  >
                    <p className="text-xs text-gray-700">
                      {(() => {
                        ...
                        const ownCount = formData.mixedDrivers.filter(
                          (d) => d.driverType === "own",
                        ).length;
                        const availableDrivers = formData.availableDrivers || 0;
                        const remainingDefault = Math.max(
                          0,
                          availableDrivers - defaultCount,
                        );
                        ...
                        if (defaultCount > availableDrivers) {
                          summary +=
                            " — Too many default drivers! Reduce to match available.";
                        }
                        return summary;
                      })()}
                    </p>
              ...
              {safeSelectedModels.length > 0
                ? safeSelectedModels.map((vehicle_id, idx) => {
                    ...
                    const mixedDriverObj = formData.mixedDrivers?.find(
                      (d) => d.vehicle_id === vehicle_id,
                    ) || {
                      driverType: "default",
                      name: "",
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          <Radio.Group
                            ...
                            onChange={(e) => {
                              ...
                              if (
                                newDriverType === "default" &&
                                shouldForceMixedDrivers
                              ) {
                                const currentDefaultCount =
                                  formData.mixedDrivers.filter(
                                    (d) => d.driverType === "default",
                                  ).length;
                                const availableDrivers =
                                  formData.availableDrivers || 0;

                                if (
                                  currentDefaultCount >= availableDrivers &&
                                  mixedDriverObj.driverType !== "default"
                                ) {
                                  toast.error(
                                    `Cannot assign more than ${availableDrivers} default drivers.`,
                                  );
                                  return;
                                }
                              }
                ...
                : shouldForceMixedDrivers && (
                    <div className="space-y-2">
                      <div className="text-sm text-gray-700">
                        Some drivers are available but not enough for all
                        vehicles.
                      </div>
                      <div className="border rounded-lg p-3 space-y-2 border-orange-200 bg-orange-50">
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-2">
                            <span className="text-sm font-medium text-gray-700">
                              Vehicle (Forced Assignment)
                            </span>
                            <span className="px-2 py-1 text-xs bg-orange-100 text-orange-800 rounded-full">
                              Own Driver
                            </span>
                          </div>
                          <Radio.Group value="own" size="small" disabled>
                            <Radio value="own">Own Driver</Radio>
                          </Radio.Group>
                        </div>
                        <Input
          ...
          {formData.driverType === "default" && !shouldForceOwnDrivers && (
            <div className="p-3 bg-gray-50 rounded-lg text-gray-600 text-sm">
              Default driver will be assigned by admin.
            </div>
          )}
        </div>
      </Form.Item>
        ...
        if (response.data.status === "success") {
          ...
          const transformedEquipment = equipmentData.map((item) => ({
            ...
            equipments_category_id:
              item.equipments_category_id || item.equipment_category_id,
          }));
  ...
  useEffect(() => {
    if (formData.forceOwnDrivers && selectedModels.length > 0) {
      ...
      if (availableDrivers > 0 && availableDrivers < totalVehicles) {
        toast.success(
          `Mixed driver mode enabled. You can assign up to ${availableDrivers} default driver(s) and provide your own drivers for the rest.`,
        );
      }
    }
  }, [
    formData.forceOwnDrivers,
    formData.forceMixedDrivers,
    selectedModels,
    formData.availableDrivers,
  ]);
  ...
  useEffect(() => {
    if (currentStep === 2) {
      ...
      const fetchCalendarData = async () => {
        try {
          // Fetch holidays
          const holidayResponse = await axios.post(`${encryptedUrl}/user.php`, {
            operation: "fetchHoliday",
          });

          if (holidayResponse.data.status === "success") {
            const formattedHolidays = holidayResponse.data.data.map(
              (holiday) => ({
          ...
          if (resourceType === "equipment") {
            ...
            const reservationResponse = await axios.post(
              `${encryptedUrl}/user.php`,
              {
                ...
                resourceType: resourceType,
                resourceIds:
                  resourceType === "venue" ? formData.venues : selectedModels,
              },
            );
  ...
  return (
    <main className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100">
      ...
      <section
        className={`w-full transition-all duration-300 ${isMobile ? "px-2 py-3 pb-24" : "p-6"}`}
      >
        <article className={`mx-auto ${isMobile ? "max-w-full" : "max-w-6xl"}`}>
          {/* Header */}
          <header
            className={`bg-white rounded-xl shadow-sm p-4 border border-gray-100 ${isMobile ? "mb-2" : "mb-6"}`}
          >
            <Button
              onClick={() => navigate(-1)}
            ...
            <h1
              className={`font-bold text-gray-900 ${isMobile ? "text-xl mt-2" : "text-3xl"}`}
            >
              Create Reservation
            </h1>
          ...
          >
            {/* Step Header */}
            <header
              className={`px-3 py-2 md:px-4 md:py-3 lg:px-6 lg:py-4 bg-gradient-to-r from-lime-900 to-green-900 border-b border-gray-100 rounded-lg shadow-sm`}
            >
              <h2
                className={`font-semibold text-white  ${isMobile ? "text-lg" : "text-xl md:text-2xl"}`}
              >
                {currentStep === 0 && "Select Resource Type"}
                {currentStep === 1 &&
                  `Select ${resourceType === "venue" ? "Venue" : resourceType === "vehicle" ? "Vehicle" : "Equipment"}`}
            ...
            <article className={isMobile ? "p-2" : "p-4 md:p-6"}>
              <div
                className={`mx-auto ${isMobile ? "max-w-full" : "max-w-2xl md:max-w-3xl lg:max-w-4xl"}`}
              >
                {renderStepContent()}
              </div>
            </article>
            ...
            <footer className="fixed bottom-0 left-0 right-0 px-4 py-3 bg-gradient-to-r from-lime-900 to-green-900 border-t border-gray-100 z-30 shadow-sm">
  <nav className="flex flex-col sm:flex-row justify-between items-center gap-2 sm:gap-4">
    {/* Left Button Group - Always shows text */}
    <div className="flex flex-col xs:flex-row gap-2 w-full sm:w-auto">
      <AntButton
        ...
        onClick={handleBack}
        size="middle"
        className={`w-full sm:w-auto border border-gray-300 text-gray-700 bg-gradient-to-r from-gray-100 to-gray-200 hover:from-gray-200 hover:to-gray-300 disabled:opacity-50`}
      ...
      {currentStep > 0 && (
        <AntButton
          ...
          onClick={resetForm}
          size="middle"
          className="p-button-outlined border-orange-500 text-orange-600 hover:bg-orange-50 w-full sm:w-auto"
        >
          Reset Form
        </AntButton>
      )}
    ...
    {/* Right Button - Always shows text */}
    {currentStep === 4 ? (
      <AntButton
        type="primary"
        icon={
        ...
        size="middle"
        className="p-button-success bg-green-500 hover:bg-green-600 border-green-500 w-full sm:w-auto"
        disabled={loading}
      >
      ...
      <AntButton
        ...
        onClick={handleNext}
        size="middle"
        className="p-button-primary bg-gradient-to-r from-lime-600 to-green-600 hover:from-lime-700 hover:to-green-700 border-lime-600 text-white w-full sm:w-auto"
      >
        Next
      </AntButton>
    )}
      ...
      {currentStep !== 5 && isMobile && (
        <div
          className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 shadow-lg z-50 p-2 flex flex-col gap-2"
          ...
          <nav className="flex flex-col gap-2 w-full">
            ...
            {currentStep === 4 ? (
              <AntButton
                type="primary"
                icon={
                  loading ? <Spin className="mr-2" /> : <CheckCircleOutlined />
                }
                onClick={handleAddReservation}
                size="middle"
