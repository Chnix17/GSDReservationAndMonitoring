      ...
      >
        ...
        >
          ...
          >
            ...
            {message.replyTo && (
              <div
                className={`mb-2 px-3 py-2 rounded-lg text-xs font-medium ${isOwn ? "bg-primary-dark/30 text-white" : "bg-gray-100 text-gray-600"}`}
              >
                <p className="flex items-center gap-1 mb-1">
                  <FiCornerUpRight className="w-3 h-3" />
                  <span>
                    {isOwn && message.replyTo.senderName === currentUser.name
            ...
            <p
              className={`text-sm leading-relaxed ${isOwn ? "text-white" : "text-gray-800"}`}
            >
              {message.text}
            </p>
            <div className="flex items-center justify-end gap-2 mt-1">
              ...
              {isOwn && (
                <span
                  className={`text-[10px] ${isOwn ? "text-white/70" : "text-gray-400"} flex`}
                >
                  {renderStatus()}
                </span>
              )}
...
const Chat = () => {
              ...
              <div className="flex items-center gap-3">
                <div className="relative">
                  <img
                    src={
                      activeConversation &&
                      getAvatarUrl(activeConversation.picture)
                    }
  ...
  const memoizedSearchEmails = useCallback(
    async (query) => {
      ...
      try {
        ...
        if (result.status === "success" && Array.isArray(result.data)) {
          const filteredResults = result.data.filter(
            (user) => user.users_id !== currentUser.id,
          );
          ...
          if (data.message && (data.sender_id || data.receiver_id)) {
            ...
            setMessages((prev) => {
              ...
              const newMessage = {
                ...
                senderName:
                  data.sender_name ||
                  (data.sender_id === parseInt(currentUser.id)
                    ? currentUser.name
  ...
  const renderConnectionStatus = () => {
    ...
    return (
      <div className="absolute left-0 right-0 top-[94px] z-20 px-4 py-1.5 flex justify-center">
        <motion.div
          initial={{ opacity: 0, y: -10 }}
          animate={{ opacity: 1, y: 0 }}
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-gradient-to-br from-green-100 to-white">
      ...
      <div className="flex-1 flex flex-col h-full min-w-[300px] mt-20 md:mt-[7rem] mx-1.5 md:mx-2 rounded-xl">
        {renderConnectionStatus()}

        {viewMode === "list" ? (
          // List View
          <div className="flex flex-col h-full overflow-hidden rounded-xl">
            <div className="bg-gradient-to-r from-primary/90 to-primary-dark/90 text-white shadow-md rounded-xl">
          ...
          // Conversation View
          <div className="flex flex-col h-full bg-gradient-to-br from-white via-gray-50/50 to-green-100/20 rounded-xl">
            {/* Chat Header - only show on desktop */}
            {!isMobile && renderChatHeader()}

            {/* Messages container */}
            <div className="flex-1 overflow-y-auto px-4 py-6 pb-24 bg-gradient-to-br from-white/90 via-gray-50/90 to-green-100/50 rounded-xl">
