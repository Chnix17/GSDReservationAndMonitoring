...
import { Modal, Tag, Alert, Table, Tooltip, Input, Radio, Space, Empty, Pagination, Button, Spin, Progress, Drawer } from 'antd';
import {
    CarOutlined,
    BuildOutlined,
    ToolOutlined,
...
import AssignOptionModal from './AssignOptionModal';
import RescheduleModal from './reschedule_modal';


const ReservationRequestsComponent = () => {
    ...
    const handleAccept = async (vehicleDriverAssignments = {}) => {
        ...
        try {
            ...
            // Insert units for department approvers OR for the final approver in the sequence
            if (((isDepartmentApprover && isDeptApprovalPending) || isFinalApprover) && reservationDetails?.equipment && reservationDetails.equipment.length > 0) {
                try {
    ...
    const handleAcceptWithOverride = async () => {
        ...
        try {
            ...
            if (((isDepartmentApprover && isDeptApprovalPending) || isFinalApproverOverride) && reservationDetails?.equipment && reservationDetails.equipment.length > 0) {
                try {
                    // Format the data to match backend expectations
                    const equipIds = reservationDetails.equipment.map(eq => parseInt(eq.equipment_id));
                    const quantities = reservationDetails.equipment.map(eq => parseInt(eq.quantity));
...
export default ReservationRequestsComponent;
